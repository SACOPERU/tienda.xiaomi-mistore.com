...
function quantity_partner($product_id, $color_id = null, $size_id = null){
    ...
    if($size_id){
        $size = Size::find($size_id);
        $quantity_partner = $size->colors->find($color_id)->pivot->quantity_partner;

    }elseif($color_id){
        $quantity_partner = $product->colors->find($color_id)->pivot->quantity_partner;

    }else{
        $quantity_partner = $product->quantity_partner;
...
function qty_available($product_id, $color_id = null, $size_id = null){
    ...
    return quantity_partner($product_id, $color_id, $size_id) - qty_added($product_id, $color_id, $size_id);
...
function discount($item){
    ...
    $qty_available = qty_available($item->id, $item->options->color_id, $item->options->size_id, $item->options->sku);
